        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-geo-border">
              ...
              <th className="px-6 py-3 text-center text-xs font-semibold text-txt-secondary uppercase tracking-wider">총 수량</th>
              <th className="px-6 py-3 text-center text-xs font-semibold text-txt-secondary uppercase tracking-wider">등록</th>
              <th className="px-6 py-3 text-center text-xs font-semibold text-txt-secondary uppercase tracking-wider">미등록</th>
              <th className="px-6 py-3 text-center text-xs font-semibold text-txt-secondary uppercase tracking-wider">입고일</th>
              <th className="px-6 py-3 text-center text-xs font-semibold text-txt-secondary uppercase tracking-wider">액션</th>
            </tr>
          </thead>
          <tbody>
            {isLoading && (
            ...
            {!isLoading && series?.map((s: any) => {
              ...
              const allDone = total > 0 && unregistered === 0;
              return (
                <tr key={s.series_id || s.id} className="border-b border-geo-border/50 last:border-0 hover:bg-geo-card-hover transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm font-mono text-txt-secondary text-center">{total}</td>
                  <td className="px-6 py-4 text-center">
                    <span className="text-sm font-mono text-status-green">{registered}</span>
                  </td>
                  <td className="px-6 py-4 text-center">
                    <span className={"text-sm font-mono " + (unregistered > 0 ? "text-status-red" : "text-txt-muted")}>{unregistered}</span>
                  </td>
                  <td className="px-6 py-4 text-sm text-txt-muted font-mono text-center">
                  ...
                  <td className="px-6 py-4 text-center">
                    {unregistered > 0 ? (
                      <button
                        ...
                        className="text-xs px-3 py-1.5 rounded-lg bg-status-purple/15 text-status-purple font-medium hover:bg-status-purple hover:text-white transition-all"
                      >
                        정품등록 →
                      </button>
                    ) : allDone ? (
                      <span className="text-xs px-3 py-1.5 rounded-lg bg-status-green-dim text-status-green font-medium">
                        등록완료 ✓
                      </span>
                    ) : null}
                  </td>
